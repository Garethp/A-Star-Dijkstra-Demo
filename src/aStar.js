export default class {
  ...
  heuristic(currentCell, endCell) {
    const xDistance = Math.abs(endCell.pos.x - currentCell.pos.x);
    const yDistance = Math.abs(endCell.pos.y - currentCell.pos.y);
  ...
  step() {
    ...
    openList.forEach(function (value, index) {
      ...
      if (
        value.estimatedTotalDistance <
        openList[lowestIndex].estimatedTotalDistance
      ) {
        lowestIndex = index;
        return;
      }
    ...
    const neighbors = this.getNeighbors(this.grid.getRows(), currentNode);
    for (let neighbor of neighbors) {
      ...
      let distanceTraveledIsShorted = false;
      const distanceTraveled = currentNode.distanceTraveled + neighbor.time;
      ...
      if (distanceTraveledIsShorted) {
        ...
        neighbor.estimatedTotalDistance =
          neighbor.distanceLeft + neighbor.distanceTraveled;
      }
    }
